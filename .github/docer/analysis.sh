#!/usr/bin/env bash
#
# Vendored + patched fork of wesleytanws/DOCER_tool analysis.sh.
# Changes from upstream (all marked with `# PATCH:`):
#   1. Page discovery targets CLAUDE.md instead of README.md.
#   2. Code search excludes the instruction file itself AND other docs
#      (*.md, docs/**) so a stale reference echoed in another doc can't
#      mask real rot in the code (DOCER's cross-document blind spot).
#   3. code_elements is written to a temp file ($CE) and passed to
#      `git grep -f "$CE"` instead of a `<(process substitution)`, which
#      git cannot read via /proc on some platforms.
#   4. exclude_path points at the vendored tool/.DOCER_exclude.

run_evaluate() {

    exclude_path="tool/.DOCER_exclude"   # PATCH: vendored exclude list
    ...
    mkdir -p output
    CE="$PWD/output/.code_elements.tmp"   # PATCH: temp file for git grep -f
    ...
    while IFS= read -r -d $'\0' page; do
        ...
        # PATCH: match CLAUDE.md (was README.md) in the source repo root
        awk 'BEGIN { RS="\0"; ORS="\0" }; { print 0, $0 }' <(
            grep -axzF CLAUDE.md <( # Match CLAUDE.md in the root directory
...
evaluate_revision() {
    ...
    # PATCH: persist code_elements so git grep can read them via a real file
    printf '%s' "$code_elements" > "$CE"

    # List of code elements in the repository (excluding docs)
    # that match the code elements found in the documentation and file names
    matched_elements="$(
        sort <({
            # Search for code elements in the documentation
            # PATCH: -f "$CE" (was process substitution); exclude the doc + other docs
            git -C "$repo_dir" grep -howFI -f "$CE" "$rev_SHA" -- ':!*.md' ':!docs/**';
    ...
    while IFS=: read -r -d '' SHA file_name; read -d '' line_number; read -r code_element; do
        ...
        # PATCH: -f "$CE" (was process substitution); exclude the doc + other docs
        git -C "$repo_dir" grep -aznowFI -f "$CE" "$rev_SHA" -- ':!*.md' ':!docs/**'
    ) >> "output/sources.csv"
